...
class _DiaryScreenState extends State<DiaryScreen> {
  ...
  Future<void> loadEntryUrls() async {
    final prefs = await SharedPreferences.getInstance();
    final Map<String, String>? urls = prefs.getStringList('entryUrls')
        ?.asMap()
        .cast<String, String>();
    if (urls != null) {
  ...
  Future<void> _generateEntryUrl() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    prefs.setStringList('entryUrls',
        entryUrls.entries.map((e) => '${e.key}:${e.value}').toList());

    // Share the URL
    if (await canLaunch(entryUrl)) {
      await launch(entryUrl);
    } else {
      print('Could not launch $entryUrl');
    }
  }
